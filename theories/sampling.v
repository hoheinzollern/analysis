...
(* TODO: move *)
Lemma ln_div {R : realType} : {in Num.pos &, {morph ln (R:=R) : x y / (x / y)%R >-> (x - y)%R}}.
Proof.
...
Lemma le01_expR_ge1Dx {R : realType} (x : R) : (-1 <= x <= 0 -> 1 + x <= expR x)%R.
Proof.
move=> /andP [N1x x0].
...
rewrite -(@integral_distribution _ _ _ _ _ (EFin \o normr))//; last first.
  ...
  exact: measurableT_comp.
rewrite bX.1.
rewrite ge0_integral_pushforward/=; last 2 first.
  exact: measurableT_comp.
