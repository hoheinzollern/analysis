...
(*                           It uses the sets approx_A and approx_B.          *)
...
Section mfun_sigmaRing_realType.
Context {rT : realType}.
...
Definition approx_A n k := if (k < n * 2 ^ n)%N then
  D `&` [set x | f x \in EFin @` [set` I n k]] else set0.

Definition approx_B n := D `&` [set x | n%:R%:E <= f x]%E.

Local Notation A := approx_A.
Local Notation B := approx_B.
...
by apply/emeasurable_fun_sum => i; exact/measurable_EFinP.
Qed.
...
transitivity (limn (fun n => \int[pushforward mu mphi]_x (f_ n x)%:E)).
  ...
  - by move=> y _ m n mn; rewrite lee_fin; apply/lefP/nd_nnsfun_approx.
...
have cvg_f_ (m : {measure set T -> \bar R}) :
  ...
  by move=> t Dt; rewrite lee_fin; apply/lefP/nd_nnsfun_approx.
